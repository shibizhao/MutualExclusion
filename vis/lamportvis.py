        ...
        for i in range(max_layer_num+1):
            for j in layer_key[i]:
                ...
                cv2.putText(canvas, self.gen_label(self.globalEventBuffer[proc_id][event_idx]), \
                            (line_sx + (i+2) * x_stride - x_stride // 3, y_stride * proc_id + line_sy + (1 - 2 * (i%2)) * y_stride // 4), cv2.FONT_HERSHEY_SIMPLEX, 0.75, black, thickness=2)
...
def visual_wrapper(proc_num, filename="lamport_visual.png"):
