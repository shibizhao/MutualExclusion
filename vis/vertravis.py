...
class Controller():
    ...
    def init(self):
        ...
        self.globalEventMap    = {}
    def add_message(self, key, proc_id, event_idx):
        # self.globalEventMap[key] = {}
        self.globalEventMap[key] = (proc_id, event_idx)
    def ask_message(self, key):
        if key not in self.globalEventBuffer.keys():
            return self.globalEventMap[key]["proc_id"], self.globalEventMap[key]["event_idx"]
    def ask_event_idx(self, proc_id):
    ...
    def visual_opencv(self):
        ...
        for i in range(self.proc_num):
            for j in range(len(self.globalEventBuffer[i])):  
                if len(self.globalEventBuffer[i][j].prev) == 0:
                    ...
                    # self.globalEventBuffer[i][j].visit = True
        ...
        line_sy = y_stride * 2
        canvas = np.ones((x_stride * (max_layer_num + 10), y_stride * (proc_num + 3),3), dtype='uint8')
        canvas = canvas * 255
        for i in range(self.proc_num):
            black = (0,0,0)
            cv2.line(canvas, (y_stride * i + line_sy, line_sx), (y_stride * i + line_sy,line_ex ), black, thickness=2)
            cv2.putText(canvas, 'proc_' + str(i), (i * y_stride + line_sy - y_stride // 4, line_sx - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, black, thickness=2)
        ...
        for i in range(max_layer_num+1):
            for j in layer_key[i]:
                ...
                proc_id = t[0]
                event_idx = t[1]
                pos = (y_stride * proc_id + line_sy, line_sx + (i+2) * x_stride + int(np.random.rand() * 40))
                ...
                cv2.putText(canvas, self.gen_label(self.globalEventBuffer[proc_id][event_idx]), \
                            (y_stride * proc_id + line_sy + 10, line_sx + (i+2) * x_stride - x_stride // 8), cv2.FONT_HERSHEY_SIMPLEX, 0.75, black, thickness=2)
...
def read_log(name, id):
    ...
    with open(name, "r") as fp:
        ...
        for line in content:
            ...
            event_idx = globalController.ask_event_idx(id)
            ...
            if cur_key in globalController.globalEventMap.keys():
                ...
                # print(line)
                # print(globalController.globalEventBuffer[p_id][e_idx-1].type[0] == int(EventType.ENTER)
                assert globalController.globalEventBuffer[p_id][e_idx-1].type[0] == EventType.ENTER
                # if basic_feature[1] == "from":
                #     prev_key = basic_feature[2]
                #     globalController.globalEventBuffer[proc_id][event_idx].add_prev(prev_key)
                # if basic_feature[1] == "to":
                #     prev_key = basic_feature[2]
                #     globalController.globalEventBuffer[proc_id][event_idx].add_prev(prev_key)
...
def visual_wrapper(proc_num):
    globalController = Controller(proc_num=proc_num)
    for i in range(proc_num):
        read_log("./log/" + str(i) + "_log.txt", i)
    globalController.visual_opencv()